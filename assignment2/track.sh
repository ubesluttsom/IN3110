...
case "$cmd" in
  start)
    if [ "$prevcmd" == "LABEL" ] ; then
      echo "Can't start new task concurrent to previous."; exit 0
    else
      echo "START $(date +'%a %b %e %H:%M:%S %Z %Y')" | tee -a $LOGFILE
      if [ "$label" == "" ]; then
        label='<null>'
      fi
      echo "LABEL $label" | tee -a $LOGFILE; exit 1
    fi ;;
  stop)
    if [ "$prevcmd" != "LABEL" ] ; then
      echo "No current task."; exit 0
    else
  ...
  log)
    ...
    do
      ...
      # `cut`:  extract time (to spesific format for `date` below)
