...
def numba_color2gray(inputfile, level=1.0):
  ...
  def f(image):
    ...
    # multiplied with a pixel vector. I state the `1.0` case explicitly, to
    # minimize miniscule floating point errors; it's important that the row
    # sum is exactly 1, else you will see faint colors in the resulting image.
    if level == 1.0:
      gray_matrix = \
          [[0.07, 0.72, 0.21],
           [0.07, 0.72, 0.21],
    ...
    # need the innermost loop, and only need to do one sum per pixel (not
    # three).
    if level == 1.0:
